...
public class Ville {
	...
	// Variables de classe :
	public static int nbInstances = 0;					// Une variables de classe est toujours déclarée "static"
	...
	public Ville(String ville, String pays, int habitants) throws NbHabitantsException, NomVilleException {
			...
			if (ville.length()<3) {
				throw new NomVilleException("Le nom de la ville doit faire plus de 3 carractères (vous avez entré \"" + ville + "\") !");
			} else {
				this.nomVille = ville;
				this.nomPays = pays;
	...
	public static int getNbInstances () {					// Toutes les méthodes de classe n'utilisant que des variables de classe sont declarées static"
	...
	public void comparerHab (Ville v) {
		...
		int b = v.getNbHab();
		if (a == b)
			System.out.println(this.getNom() + " et " + v.getNom() + " ont le même nombre d'habitants, soit  " + a + ".");
		else if (a > b)
	...
	public String toString(){
		  return this.nomVille+" est une ville de "+this.nomPays+" qui comporte "+this.nbHabitants+" habitants et est donc de catégorie "+this.categorie+".";
	}
